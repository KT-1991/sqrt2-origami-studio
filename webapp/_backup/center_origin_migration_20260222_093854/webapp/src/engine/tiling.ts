...
function projectPointToSymmetry(p: Point, symmetry: "axis" | "pair_anchor"): Point {
  let x = Math.min(1.0, Math.max(0.0, p[0]));
  let y = Math.min(1.0, Math.max(0.0, p[1]));
  if (symmetry === "axis") {
    let t = 0.5 * (x + y);
...
function latticeValues(den: number, coeffLimit: number): number[] {
  const key = `${den}:${coeffLimit}`;
  ...
  const m = coeffLimit * den;
  for (let a = -m; a <= m; a += 1) {
    for (let b = -m; b <= m; b += 1) {
      const v = (a + b * root2) / den;
      if (v >= -1e-9 && v <= 1.0 + 1e-9) {
        vals.add(Math.min(1.0, Math.max(0.0, v)));
      }
    }
  }
  vals.add(0.0);
...
function snapIndependent(
  ...
  const vals = latticeValues(den, coeffLimit);
  const out: Record<string, Point> = {};
  for (const v of vars_) {
...
function solveKadoLayoutResolved(input: TilingRunInputResolved): TilingState {
  ...
  const warmHints = new Map<string, Record<string, Point>>();

  for (let step = 0; step < input.alphaSteps; step += 1) {
    const alpha = 0.5 * (lo + hi);
    let bestTrial: TilingState | undefined;
    let feasibleAny = false;

    for (const den of input.denCandidates) {
      for (const coeff of input.coeffCandidates) {
        const key = `${den}:${coeff}`;
        let startHint: Record<string, Point> | undefined;
        if (input.warmStart) {
          startHint = warmHints.get(key);
        }
        ...
        if (input.warmStart) {
          warmHints.set(key, clonePointMap(indepCont));
        }

        const indepSnap = snapIndependent(indepCont, vars_, den, coeff);
        const indepFix = localRepairSnap(indepSnap, vars_, specs, alpha, den, coeff);
        ...
        if (pen <= 1e-10) {
          ...
          if (
            !bestTrial ||
            cur.cornerHits > bestTrial.cornerHits ||
            (cur.cornerHits === bestTrial.cornerHits &&
              cur.contactScore > bestTrial.contactScore)
          ) {
            bestTrial = cur;
          }
        }
  ...
  if (!best) {
    return {
      ok: false,
      alpha: 0.0,
      den: input.denCandidates[0],
      coeffLimit: input.coeffCandidates[0],
      centers: {},
      cornerHits: 0,
      contactScore: 0.0,
...
export function runTiling(input: TilingRunInput): TilingState {
  ...
  return solveKadoLayoutResolved(resolved);
}
