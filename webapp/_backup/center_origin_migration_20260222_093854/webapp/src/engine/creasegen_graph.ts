...
export class GridCreaseGraph {
  ...
  *iterEdges(): Iterable<[number, number]> {
  ...
  initSquareBoundary(): [number, number, number, number] {
    const v0 = this.addVertex(point(ZERO, ZERO));
    const v1 = this.addVertex(point(ONE, ZERO));
    const v2 = this.addVertex(point(ONE, ONE));
    const v3 = this.addVertex(point(ZERO, ONE));
