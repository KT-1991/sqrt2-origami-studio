...
export interface TilingLatticeConfig {
  aMax: number;
  bMax: number;
  kMax: number;
}
...
export interface TilingRunInput {
  specs: KadoSpec[];
  // Paper center in world coordinates. If omitted, defaults to (1/2, 1/2).
  originOffset?: PointE;
  lattice?: TilingLatticeConfig;
  seed?: number;
  alphaSteps?: number;
  packRestarts?: number;
  packIters?: number;
...
export interface TilingState {
  ...
  alpha: number;
  lattice: TilingLatticeConfig;
  // Legacy summary fields for compatibility in UI/log output.
  den: number;
  coeffLimit: number;
