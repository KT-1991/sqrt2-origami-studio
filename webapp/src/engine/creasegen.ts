...
import {
  internalToWorldPoint,
  internalToWorldVec2,
  normalizeOriginOffsetWorld,
  worldToInternalPoint,
} from "./paper_frame";
...
export function runCreasegen(input: CreaseBuildInput): CreaseRunResult {
  ...
  const originOffset = normalizeOriginOffsetWorld(input.originOffset);
  const corners = input.corners.map((p) => worldToInternalPoint(p, originOffset));
  if (corners.length < 3) {
  ...
  if (input.seedSegments) {
    ...
    for (const seg of input.seedSegments) {
      ...
      const fromLocal = worldToInternalPoint(seg.from, originOffset);
      const toLocal = worldToInternalPoint(seg.to, originOffset);
      const fromKey = pointKey(fromLocal);
      const toKey = pointKey(toLocal);
      if (fromKey === toKey) {
        continue;
      }
      ...
      seen.add(key);
      initialSeedSegments.push({ from: fromLocal, to: toLocal });
    }
  }
  ...
  const graphWorld: CreaseGraphMem = {
    ...graph,
    vertices: graph.vertices.map((v) => ({
      ...v,
      point: internalToWorldPoint(v.point, originOffset),
      pointApprox: internalToWorldVec2(v.pointApprox, originOffset),
    })),
  };
