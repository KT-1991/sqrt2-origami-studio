...
class GridCreaseGraph:
    def __init__(
        ...
        self.face_dirty: bool = True
        self.face_index_version: int = -1
        self.face_query_since_dirty: int = 0
        self.face_build_threshold: int = FACE_INDEX_BUILD_THRESHOLD
        self.edge_by_id: List[Tuple[int, int]] = []
        self.edge_id: Dict[Tuple[int, int], int] = {}
        self.dedge_id: Dict[Tuple[int, int], int] = {}
        self.he_origin: List[int] = []
        self.he_twin: List[int] = []
        self.he_next: List[int] = []
        self.dedge_face: List[int] = []
        self.face_edge_ids: List[List[int]] = []
        self.vertex_outgoing_sorted: Dict[int, List[int]] = {}
        self.vertex_outgoing_angles: Dict[int, List[float]] = {}
    ...
    def _ensure_face_index(self) -> None:
        if not self.face_dirty and self.face_index_version == self.edge_version:
            return
        ...
        self.edge_id.clear()
        self.dedge_id.clear()
        self.he_origin.clear()
        self.he_twin.clear()
        self.he_next.clear()
        self.dedge_face.clear()
        self.face_edge_ids.clear()
        self.vertex_outgoing_sorted.clear()
        self.vertex_outgoing_angles.clear()
        if not self.edges:
            self.face_index_version = self.edge_version
            return

        pos_map: Dict[int, Dict[int, int]] = {}
        for v in self.active_vertices:
            nbrs = list(self.adj.get(v, set()))
            if not nbrs:
                continue
            vx, vy = self.points_f[v]
            angle_nbr = [
                (atan2(self.points_f[u][1] - vy, self.points_f[u][0] - vx), u)
                for u in nbrs
            ]
            angle_nbr.sort(key=lambda x: x[0])
            sorted_nbrs = [u for _, u in angle_nbr]
            sorted_angles = [a for a, _ in angle_nbr]
            self.vertex_outgoing_sorted[v] = sorted_nbrs
            self.vertex_outgoing_angles[v] = sorted_angles
            pos_map[v] = {u: i for i, u in enumerate(sorted_nbrs)}

        self.edge_by_id = list(self.edges)
        self.edge_id = {e: idx for idx, e in enumerate(self.edge_by_id)}
        half_edge_count = 2 * len(self.edge_by_id)
        self.he_origin = [0] * half_edge_count
        self.he_twin = [0] * half_edge_count
        self.he_next = [-1] * half_edge_count
        for idx, (i, j) in enumerate(self.edge_by_id):
            did0 = 2 * idx
            did1 = did0 + 1
            self.dedge_id[(i, j)] = did0
            self.dedge_id[(j, i)] = did1
            self.he_origin[did0] = i
            self.he_origin[did1] = j
            self.he_twin[did0] = did1
            self.he_twin[did1] = did0
        self.dedge_face = [-1] * half_edge_count

        unvisited: Set[int] = set(range(half_edge_count))
        max_steps = max(1, half_edge_count + 5)
        ...
        while unvisited:
            ...
            for _ in range(max_steps):
                ...
                face_cycle.append(cur)
                a = self.he_origin[cur]
                b = self.he_origin[self.he_twin[cur]]
                nbrs = self.vertex_outgoing_sorted.get(b)
                if not nbrs:
                    break
                idx_prev = pos_map[b].get(a)
                if idx_prev is None:
                    break
                nxt = nbrs[(idx_prev - 1) % len(nbrs)]
                nxt_did = self.dedge_id.get((b, nxt))
                if nxt_did is None:
                    break
                self.he_next[cur] = nxt_did
                cur = nxt_did
                if cur == start:
                    break
            if not face_cycle:
                continue
            fid = len(self.face_edge_ids)
            undirected_edge_ids: List[int] = []
            seen_eid: Set[int] = set()
            for did in face_cycle:
                self.dedge_face[did] = fid
                eid = did // 2
                if eid in seen_eid:
                    continue
                seen_eid.add(eid)
                undirected_edge_ids.append(eid)
            self.face_edge_ids.append(undirected_edge_ids)
        self.face_index_version = self.edge_version

    def _faces_for_ray_at_vertex(self, origin_v: int, dir_idx: int) -> List[int]:
        self._ensure_face_index()
        nbrs = self.vertex_outgoing_sorted.get(origin_v)
        angs = self.vertex_outgoing_angles.get(origin_v)
        if not nbrs or not angs:
            return []
        theta = _angle_of_dir_idx(dir_idx)
        k = bisect_right(angs, theta) - 1
        if k < 0:
            k = len(angs) - 1
        # Degenerate case: direction exactly on an existing edge direction.
        if abs(theta - angs[k]) <= 1e-12:
            return []
        out: List[int] = []
        u0 = nbrs[k]
        did0 = self.dedge_id.get((origin_v, u0))
        f0 = -1 if did0 is None else self.dedge_face[did0]
        if f0 >= 0:
            out.append(f0)
        u1 = nbrs[(k + 1) % len(nbrs)]
        did1 = self.dedge_id.get((origin_v, u1))
        f1 = -1 if did1 is None else self.dedge_face[did1]
        if f1 >= 0 and f1 not in out:
            out.append(f1)
        return out
    ...
    def _first_hit_edge_from_face_id(
        self,
        origin_v: int,
        dir_idx: int,
        face_id: int,
    ) -> Optional[Tuple[Qsqrt2, int, int, int, PointE]]:
        if face_id < 0 or face_id >= len(self.face_edge_ids):
            return None
        edge_ids = self.face_edge_ids[face_id]
        if not edge_ids:
            return None
        return self._first_hit_edge_from_candidates(
            origin_v,
            dir_idx,
            (self.edge_by_id[eid] for eid in edge_ids),
        )
    ...
    def first_hit_edge(self, origin_v: int, dir_idx: int) -> Optional[Tuple[int, int, int, PointE]]:
        ...
        if origin_v in self.active_vertices and origin_v not in self.ray_dirty:
            ...
            if row is not None:
                return row[dir_idx]
        # If the origin has degree < 2, face lookup cannot narrow candidates well.
        # Skip face-index work and use global hit directly.
        if len(self.adj.get(origin_v, ())) < 2:
            hit = self._first_hit_edge_global(origin_v, dir_idx)
            if hit is None:
                return None
            _, i, j, hit_pos, p = hit
            return (i, j, hit_pos, p)
        # Face index is expensive to rebuild on every topology change.
        # While dirty, defer rebuilding until enough ray queries accumulate.
        if self.face_dirty:
            self.face_query_since_dirty += 1
            if self.face_query_since_dirty < self.face_build_threshold:
                hit = self._first_hit_edge_global(origin_v, dir_idx)
                if hit is None:
                    return None
                _, i, j, hit_pos, p = hit
                return (i, j, hit_pos, p)
            self._ensure_face_index()
        elif self.face_index_version != self.edge_version:
            self._ensure_face_index()
        # Face-index path: test only boundary edges of the entered face.
        best_face_hit: Optional[Tuple[int, int, int, PointE]] = None
        best_face_t: Optional[Qsqrt2] = None
        for face_id in self._faces_for_ray_at_vertex(origin_v, dir_idx):
            hit = self._first_hit_edge_from_face_id(origin_v, dir_idx, face_id)
            if hit is None:
                continue
            t, i, j, hit_pos, p = hit
            if best_face_t is None or _q2_cmp(t, best_face_t) < 0:
                best_face_t = t
                best_face_hit = (i, j, hit_pos, p)
        if best_face_hit is not None:
            return best_face_hit
...
def clone_graph(g: GridCreaseGraph) -> GridCreaseGraph:
    ...
    h.face_dirty = g.face_dirty
    h.face_index_version = g.face_index_version
    h.face_query_since_dirty = g.face_query_since_dirty
    h.face_build_threshold = g.face_build_threshold
    if g.face_dirty:
        # While dirty these structures are stale and unused until _ensure_face_index().
        # Skip copying large face-index payloads in DFS clones.
        h.edge_by_id = []
        h.edge_id = {}
        h.dedge_id = {}
        h.he_origin = []
        ...
        h.dedge_face = []
        h.face_edge_ids = []
        h.vertex_outgoing_sorted = {}
        h.vertex_outgoing_angles = {}
    else:
        h.edge_by_id = list(g.edge_by_id)
        h.edge_id = dict(g.edge_id)
        h.dedge_id = dict(g.dedge_id)
        h.he_origin = list(g.he_origin)
        h.he_twin = list(g.he_twin)
        h.he_next = list(g.he_next)
        h.dedge_face = list(g.dedge_face)
        h.face_edge_ids = [list(es) for es in g.face_edge_ids]
        h.vertex_outgoing_sorted = {k: list(vs) for k, vs in g.vertex_outgoing_sorted.items()}
        h.vertex_outgoing_angles = {k: list(vs) for k, vs in g.vertex_outgoing_angles.items()}
    return h
...
def adopt_graph_state(dst: GridCreaseGraph, src: GridCreaseGraph) -> None:
    ...
    dst.face_dirty = src.face_dirty
    dst.face_index_version = src.face_index_version
    dst.face_query_since_dirty = src.face_query_since_dirty
    dst.face_build_threshold = src.face_build_threshold
    if src.face_dirty:
        dst.edge_by_id = []
        dst.edge_id = {}
        dst.dedge_id = {}
        ...
        dst.dedge_face = []
        dst.face_edge_ids = []
        dst.vertex_outgoing_sorted = {}
        dst.vertex_outgoing_angles = {}
    else:
        dst.edge_by_id = list(src.edge_by_id)
        dst.edge_id = dict(src.edge_id)
        dst.dedge_id = dict(src.dedge_id)
        dst.he_origin = list(src.he_origin)
        dst.he_twin = list(src.he_twin)
        dst.he_next = list(src.he_next)
        dst.dedge_face = list(src.dedge_face)
        dst.face_edge_ids = [list(es) for es in src.face_edge_ids]
        dst.vertex_outgoing_sorted = {k: list(vs) for k, vs in src.vertex_outgoing_sorted.items()}
        dst.vertex_outgoing_angles = {k: list(vs) for k, vs in src.vertex_outgoing_angles.items()}
...
def _is_collinear_with_axis_dir(d: int, axis_d: int) -> bool:
    return d == axis_d or d == ((axis_d + ANGLE_COUNT // 2) % ANGLE_COUNT)


def _iter_incident_nonboundary_dir_neighbors(
    g: GridCreaseGraph,
    v: int,
    skip_u: int,
    ...
    for u in g.adj.get(v, set()):
        if u == skip_u:
            continue
        e = g._norm_edge(v, u)
        if (not allow_boundary) and e in g.boundary_edges:
            continue
        d = _edge_dir_from(g, v, u)
        if d is None:
            continue
        out.append((d, u))
    return out
...
def _short_bridge_exists(
    ...
    if src == dst:
        return False
    e0 = g._norm_edge(src, dst)
    if e0 in g.edges and (allow_boundary or e0 not in g.boundary_edges):
        return True
    if max_hops <= 1:
        return False
    # Lightweight 2-hop check (sufficient for current relaxed 4/6 cycle probe).
    for m in g.adj.get(src, set()):
        if m == dst:
            continue
        e1 = g._norm_edge(src, m)
        if (not allow_boundary) and e1 in g.boundary_edges:
            continue
        e2 = g._norm_edge(m, dst)
        if e2 in g.edges and (allow_boundary or e2 not in g.boundary_edges):
            return True
    return False


def _edge_target_even_cycle_len(
    g: GridCreaseGraph,
    e: Tuple[int, int],
    allow_boundary_cycle: bool = True,
    bridge_max_hops: int = 2,
) -> int:
    a, b = g._norm_edge(e[0], e[1])
    if (a, b) not in g.edges:
        return 0
    if (a, b) in g.boundary_edges:
        return 0

    axis_d = _edge_dir_from(g, a, b)
    if axis_d is None:
        return 0

    # Local edge-direction neighborhoods around the seed edge.
    na = _iter_incident_nonboundary_dir_neighbors(g, a, skip_u=b, allow_boundary=allow_boundary_cycle)
    nb = _iter_incident_nonboundary_dir_neighbors(g, b, skip_u=a, allow_boundary=allow_boundary_cycle)
    if not na or not nb:
        return 0

    nb_by_dir: Dict[int, List[int]] = {}
    for d, v in nb:
        nb_by_dir.setdefault(d, []).append(v)

    # 4-cycle check:
    # a--u1 and b--v1 are mirror directions around axis(a,b), and u1--v1 exists.
    for d1, u1 in na:
        if _is_collinear_with_axis_dir(d1, axis_d):
            continue
        md1 = _reflected_dir_idx_by_axis_dir(d1, axis_d)
        for v1 in nb_by_dir.get(md1, []):
            if _short_bridge_exists(
                g,
                u1,
                v1,
                max_hops=bridge_max_hops,
                allow_boundary=allow_boundary_cycle,
            ):
                return 4

    # 6-cycle check:
    # Extend one more mirrored step: a-u1-u2 and b-v1-v2, then u2--v2 bridge.
    for d1, u1 in na:
        if _is_collinear_with_axis_dir(d1, axis_d):
            continue
        md1 = _reflected_dir_idx_by_axis_dir(d1, axis_d)
        v1_list = nb_by_dir.get(md1, [])
        if not v1_list:
            continue

        u1_next = _iter_incident_nonboundary_dir_neighbors(g, u1, skip_u=a, allow_boundary=allow_boundary_cycle)
        if not u1_next:
            continue
        for v1 in v1_list:
            v1_next = _iter_incident_nonboundary_dir_neighbors(g, v1, skip_u=b, allow_boundary=allow_boundary_cycle)
            if not v1_next:
                continue
            v1_by_dir: Dict[int, List[int]] = {}
            for d, vv in v1_next:
                v1_by_dir.setdefault(d, []).append(vv)
            for d2, u2 in u1_next:
                if _is_collinear_with_axis_dir(d2, axis_d):
                    continue
                md2 = _reflected_dir_idx_by_axis_dir(d2, axis_d)
                for v2 in v1_by_dir.get(md2, []):
                    if _short_bridge_exists(
                        g,
                        u2,
                        v2,
...
def _edge_line_key(g: GridCreaseGraph, e: Tuple[int, int]) -> Tuple[int, int, int, int]:
    a, b = g._norm_edge(e[0], e[1])
    d = g.edge_dir_idx.get((a, b))
    if d is None:
        d = _edge_dir_from(g, a, b)
    if d is None:
        # Fallback: keep as unique bucket to avoid accidental merging.
        return (ANGLE_COUNT, a, b, 0)
    axis_d = d % (ANGLE_COUNT // 2)
    p = g.points[a]
    rx, ry = DIRS[axis_d]
    c = _cross(p.x, p.y, rx, ry)
    return (axis_d, c.a, c.b, c.k)


def _line_key_eq(a: Tuple[int, int, int, int], b: Tuple[int, int, int, int]) -> bool:
    return a[0] == b[0] and a[1] == b[1] and a[2] == b[2] and a[3] == b[3]


def _edge_center_dist2(g: GridCreaseGraph, e: Tuple[int, int]) -> float:
...
def _edge_is_deletable(g: GridCreaseGraph, u: int, v: int) -> bool:
    e = g._norm_edge(u, v)
    return e in g.edges and e not in g.boundary_edges


def _run_delete_group_transaction(
    g: GridCreaseGraph,
    delete_group: Set[Tuple[int, int]],
    corner_ids: Sequence[int],
    enforce_symmetry: bool,
    kawasaki_tol: float,
    baseline_k_bad: Optional[int] = None,
) -> Optional[Tuple[GridCreaseGraph, int]]:
    if not delete_group:
        return None
    norm_group: Set[Tuple[int, int]] = {g._norm_edge(a, b) for a, b in delete_group}
    ...
    for a, b in norm_group:
        touched.add(a)
        touched.add(b)
    before_bad, before_sum = _local_kawasaki_metric(g, touched, kawasaki_tol)
    ...
    after_bad, after_sum = _local_kawasaki_metric(trial, touched, kawasaki_tol)
...
def _best_axis_cycle_group_for_line(
    g: GridCreaseGraph,
    ...
    # key = (cycle_len, tie_break_rank, group, rep_edge)
...
def _collect_prune_axis_representatives(
    g: GridCreaseGraph,
    max_candidates: int,
    ...
    for e in candidates:
        lk = _edge_line_key(g, e)
        line_groups.setdefault(lk, []).append(e)
        rk = (_edge_center_dist2(g, e), -g.edge_birth.get(e, -1))
    ...
    for lk in ordered_lines[:line_scan_limit]:
        best = _best_axis_cycle_group_for_line(g, lk, line_groups.get(lk, []))
        if best is None:
            continue
    ...
    targeted.sort(
        key=lambda it: (
            0 if it[1] == 4 else 1,  # Prefer 4-cycles over 6-cycles.
            _edge_center_dist2(g, it[0]),
...
def _collect_axis_cycle_targets(
    g: GridCreaseGraph,
    max_candidates: int,
...
def _best_delete_group_for_vertex(
    g: GridCreaseGraph,
    v: int,
    corner_ids: Sequence[int],
    deleted: Set[Tuple[int, int]],
    ...
    incident = list(g.adj.get(v, set()))
    if not incident:
        return None

    best_key: Optional[Tuple[int, float, int, float, int, int, int]] = None
    best_group: Optional[Set[Tuple[int, int]]] = None
    before_v = _finite_kawasaki_error(g, v)
    for u in incident:
        e = g._norm_edge(v, u)
        if e in deleted:
            continue
        if e in g.boundary_edges:
            continue
        group: Set[Tuple[int, int]] = {e}
        if enforce_symmetry:
            ex = _expand_delete_edges_with_symmetry(g, group)
            if ex is None:
                continue
            group = ex
        if any(ge in g.boundary_edges for ge in group):
            continue
        if any(ge in deleted for ge in group):
            continue
        if not all(ge in g.edges for ge in group):
            continue
        ...
        after_bad, after_sum = _local_kawasaki_metric(trial, touched, kawasaki_tol)
        # Require local non-worsening to keep transaction stable.
        if after_bad > before_bad:
            continue
        if after_sum > before_sum + 1e-12:
            continue
        after_v = _finite_kawasaki_error(trial, v) if v in trial.active_vertices else 0.0
        # Prioritize reducing target vertex error, then local bad-count / residual.
        key = (
            0 if after_v + 1e-12 < before_v else 1,
            after_v - before_v,
...
def _run_delete_propagation_transaction(
    g: GridCreaseGraph,
    ...
    e0 = g._norm_edge(seed_edge[0], seed_edge[1])
    if e0 not in g.edges:
        return None
    if e0 in g.boundary_edges:
        return None
    ...
    def _apply_group(group: Set[Tuple[int, int]]) -> bool:
        for a, b in sorted(group):
            e = trial._norm_edge(a, b)
            if e not in trial.edges:
                return False
            trial.remove_edge(a, b)
            deleted.add(e)
            frontier.append(a)
    ...
    seed_group: Set[Tuple[int, int]] = {e0}
    if enforce_symmetry:
        ex = _expand_delete_edges_with_symmetry(trial, seed_group)
        if ex is None:
            return None
        seed_group = ex
    if any(e in trial.boundary_edges for e in seed_group):
        return None
    if not _apply_group(seed_group):
        return None
    _deactivate_isolated_noncorner_vertices(trial, corner_ids=corner_ids)
    if enforce_symmetry and not diagonal_symmetry_ok(trial):
        return None
    ...
    for _ in range(max(0, max_steps)):
        cur = sorted(set(frontier))
        frontier = []
        if not cur:
            break
        need = [v for v in cur if (v in trial.active_vertices and (not _is_boundary_vertex(trial, v)) and vertex_kawasaki_error(trial, v) > kawasaki_tol)]
        if not need:
            break
        # Process larger violations first.
        need.sort(key=lambda vv: _finite_kawasaki_error(trial, vv), reverse=True)
        progressed = False
        for v in need:
            group = _best_delete_group_for_vertex(
                trial,
                v=v,
    ...
    after_bad = kawasaki_score(trial, tol=kawasaki_tol)[0]
    if after_bad > base_bad:
        return None
    return (trial, len(deleted))
...
def _refresh_graph_by_pruning(
    ...
    targeted = _collect_axis_cycle_targets(h, max_candidates=max_candidates)
    if stats is not None:
        stats["prune_targeted_edges_total"] = stats.get("prune_targeted_edges_total", 0) + len(targeted)
        if probe_line_key is not None and any(_line_key_eq(lk, probe_line_key) for lk, _, _, _ in targeted):
    ...
    for lk, rep_edge, _, group in targeted[:max_candidates]:
        if lk in tried_axis:
            continue
        tried_axis.add(lk)
        is_probe = (probe_line_key is not None and _line_key_eq(lk, probe_line_key))
        if stats is not None:
        ...
        tx = _run_delete_group_transaction(
            h,
            delete_group=group,
            corner_ids=corner_ids,
            enforce_symmetry=enforce_symmetry,
...
def seed_direct_corner_connections(
    ...
    for _, u, v in pairs:
        ...
        trial = clone_graph(g)
        ok = _add_segment_with_splits_ids(trial, u, v, max_steps=64, stats=stats)
        if ok and enforce_symmetry and mirror_edge != e:
            ok = _add_segment_with_splits_ids(trial, mirror_edge[0], mirror_edge[1], max_steps=64, stats=stats)
        if ok:
            if enforce_symmetry and not diagonal_symmetry_ok(trial):
                continue
...
def _corner_condition_error_with_added_dir(g: GridCreaseGraph, v_idx: int, dir_idx: int, max_deg: float) -> float:
    px, py = g.points_f[v_idx]
    start, width = _interior_wedge(px, py)
    angs = unique_angles(incident_angles(g, v_idx, include_boundary=False))
    a = _angle_of_dir_idx(dir_idx)
    t = _norm_angle(a - start)
    if -1e-12 <= t <= width + 1e-12:
...
def _boundary_corner_promising_expand_targets(
    g: GridCreaseGraph,
    corner_ids: Sequence[int],
    max_deg: float,
    min_corner_lines: int,
    ...
    out: List[int] = []
    for v in corner_ids:
        if v not in g.active_vertices or (not _is_boundary_vertex(g, v)):
            continue
        before_err = corner_condition_error(g, v, max_deg=max_deg)
        before_lines = corner_line_count(g, v)
        if before_err <= 1e-12 and before_lines >= min_corner_lines:
            continue

        used = used_dir_indices(g, v, include_boundary=False)
        ...
        for d in admissible_dirs_for_vertex(g, v, enforce_symmetry=enforce_symmetry):
            ...
            if hit_v is None:
                continue
            line_improve = before_lines < min_corner_lines
            err_after = _corner_condition_error_with_added_dir(g, v, d, max_deg=max_deg)
        ...
        if not feasible:
            continue
        check_dirs = _topk_dirs_for_vertex(
            g,
            v_idx=v,
            dirs=feasible,
            used_dirs=used,
...
def _preserve_satisfied_corners(
    before_g: GridCreaseGraph,
    after_g: GridCreaseGraph,
    corner_ids: Sequence[int],
    max_deg: float,
    min_corner_lines: int,
    tol: float = 1e-12,
...
def violating_vertex_priority(
    ...
    interior_corners = [v for v in corner_ids if not _is_boundary_vertex(g, v)]
    ...
    def _priority_group(v: int) -> int:
        is_corner = v in cset
        is_boundary_corner = is_corner and _is_boundary_vertex(g, v)
...
def dfs_repair_corners(
    ...
    def _cached_global_score(state: GridCreaseGraph) -> Tuple[int, int, int, float, float, float]:
    ...
    def _cached_priority_corner_kawasaki(state: GridCreaseGraph) -> Tuple[int, float]:
        sk = graph_state_key(state)
    ...
    def recurse(state: GridCreaseGraph, depth: int) -> None:
        ...
        child_pool: List[Tuple[Tuple, GridCreaseGraph]] = []
        ...
        for v in priority[: max(1, priority_top_n)]:
            ...
            for d in trial_dirs:
                ...
                move_tier = _move_structure_tier(
                    state,
                    v_idx=v,
                    dir_idx=d,
                ...
                if require_corner_kawasaki:
                    ...
                    if hck[0] > ck[0]:
                        ...
                        continue
                k_increase = 1 if hsc[0] > sc[0] else 0
                child_key = (k_increase, move_tier, *hsc)
...
def make_grid_graph(
    ...
    seed_direct_corner_connections(g, corner_ids, enforce_symmetry=enforce_symmetry, stats=seed_stats)
    ...
    _add_segment_with_splits_ids(g, v00, v11, max_steps=128, stats=seed_stats)
...
def render_pattern(
    ...
    if highlight_kawasaki:
        bad_vs = [v for v in _kawasaki_target_vertex_ids(g) if vertex_kawasaki_error(g, v) > kawasaki_tol]
        if bad_vs:
...
def _expand_request_from_stats(stats: Dict[str, int]) -> Optional[Tuple[int, int, int, int, int]]:
    ...
    # Backward-compatible fallback when norm stats are not present.
    if (ran <= 0 or rbn <= 0) and (ra > 0 or rb > 0):
        fan, fbn = _required_norm_bounds_from_grid_bounds(ra, rb, rk)
        if ran <= 0:
            ran = fan
        if rbn <= 0:
            rbn = fbn
...
def _single_ray_growth_probe(
    ...
    hit_v = g.point_to_id.get(_point_key(p))
    if hit_v is None:
        return (True, 2, _required_grid_bounds_for_point(p))
    if hit_v in g.active_vertices:
        return (True, 0, None)
    return (True, 1, None)


def _move_growth_probe(
    g: GridCreaseGraph,
    v_idx: int,
    dir_idx: int,
    enforce_symmetry: bool,
) -> Tuple[bool, int, Optional[Tuple[int, int, int]]]:
    usable, grow, req = _single_ray_growth_probe(g, v_idx, dir_idx)
    if not usable:
        return (False, 2, None)
    ...
    if mv is None:
        return (False, 2, None)
    md = mirrored_dir_idx(dir_idx)
    musable, mgrow, mreq = _single_ray_growth_probe(g, mv, md)
    if not musable:
        return (False, 2, None)
    out_req = req
    if mreq is not None:
...
def _best_expand_move_for_corner(
    g: GridCreaseGraph,
    v_idx: int,
    enforce_symmetry: bool,
    ...
    if v_idx not in g.active_vertices:
        return None
    used = used_dir_indices(g, v_idx, include_boundary=False)
    ...
    for d in admissible:
        if d in used:
            continue
        usable, grow, req = _move_growth_probe(g, v_idx=v_idx, dir_idx=d, enforce_symmetry=enforce_symmetry)
        if (not usable) or grow <= 1 or req is None:
            continue
        ran, rbn = _required_norm_bounds_from_grid_bounds(req[0], req[1], req[2])
...
def _grid_required_corner_expand_request(
    g: GridCreaseGraph,
    corner_ids: Sequence[int],
    max_deg: float,
    min_corner_lines: int,
    ...
    # item: (v, need_a, need_b, need_k, need_a_norm, need_b_norm, has_nonexpand_move_flag, dir_idx)
    ...
    for v in corner_ids:
        ...
        for d in admissible:
            if d in used:
                continue
            usable, grow, req = _move_growth_probe(g, v_idx=v, dir_idx=d, enforce_symmetry=enforce_symmetry)
            if not usable:
                continue
            if grow <= 1:
...
def run(
    ...
    while True:
        ...
        _merge_search_stats(search_stats, seed_stats)
        if not seed_auto_expand:
            break
        req_seed = _expand_request_from_stats(seed_stats)
        if req_seed is None:
            break
        ...
        target_a_norm = max(a_norm_work, need_a_norm)
        target_b_norm = max(b_norm_work, need_b_norm)
        target_a = max(a_work, need_a, target_a_norm << target_k)
        target_b = max(b_work, need_b, target_b_norm << target_k)
        if (
            target_a <= a_work
            and target_b <= b_work
            and target_k <= ks
    ...
    def _search_stage(
        ...
        nonlocal a_work, b_work, a_norm_work, b_norm_work
        g_local = g_stage
        ...
        max_stage_iters = max(1, (auto_expand_max_rounds + 1) * 12)
        stage_iter = 0
        while True:
            stage_iter += 1
            if stage_iter > max_stage_iters:
                search_stats["stage_iter_limit"] = search_stats.get("stage_iter_limit", 0) + 1
                return g_local, c_local, k_local
            base_sc = global_score(
                g_local,
                corner_ids=c_local,
            ...
            if best_sc_local < base_sc:
                # Keep spending budget on coarse grid while improving.
                stall_streak = 0
                search_stats["coarse_round_improved"] = search_stats.get("coarse_round_improved", 0) + 1
                g_local = best_local
                if best_sc_local[0] == 0 and best_sc_local[1] == 0 and best_sc_local[2] == 0:
                    return best_local, c_local, k_local
                continue
            stall_streak += 1
            search_stats["coarse_round_stalled"] = search_stats.get("coarse_round_stalled", 0) + 1
            g_local = best_local
            stall_need = _effective_stall_rounds(
                active_vertices=len(g_local.active_vertices),
                base_rounds=expand_stall_rounds,
                max_nodes=max_nodes,
            )
            search_stats["stall_round_need_max"] = max(
                search_stats.get("stall_round_need_max", 0),
                stall_need,
            )
            if stall_streak < stall_need:
                continue

            # Lightweight auto-expand rule:
            # If this DFS round actually hit missing-grid failures and there are still
            # unmet boundary corners, expand immediately from round stats.
            promising_boundary = _boundary_corner_promising_expand_targets(
                best_local,
                corner_ids=c_local,
                max_deg=corner_max_deg,
                min_corner_lines=min_corner_lines,
            ...
            req_stats = _expand_request_from_stats(round_stats)
            need_a = 0
            need_b = 0
            need_k = 0
            need_a_norm = 0
            ...
            if req_stats is not None and promising_boundary:
                ...
                sel = _best_expand_move_for_corner(
                    best_local,
                    v_idx=need_corner_v,
                    enforce_symmetry=enforce_symmetry,
                )
                if sel is not None:
                    need_corner_d = sel[0]
                expand_reason = "round_missing_grid"
                ...
                req = _grid_required_corner_expand_request(
                    best_local,
                    corner_ids=c_local,
                    max_deg=corner_max_deg,
                    min_corner_lines=min_corner_lines,
                ...
                expand_reason = "grid_required_corner"
            ...
            if expand_reason == "round_missing_grid":
                target_a = max(a_work, min(need_a, a_work + 1))
                target_b = max(b_work, min(need_b, b_work + 1))
                target_a_norm = max(a_norm_work, min(need_a_norm, a_norm_work + 1))
                target_b_norm = max(b_norm_work, min(need_b_norm, b_norm_work + 1))
            else:
            ...
            k_changed = target_k > cur_k
            ab_changed = target_a > cur_a or target_b > cur_b
            norm_changed = target_a_norm > cur_a_norm or target_b_norm > cur_b_norm
            ...
            if need_corner_v in best_local.active_vertices:
                forced_corner_key = _point_key(best_local.points[need_corner_v])
            ...
            if forced_corner_key is not None and need_corner_d >= 0:
            ...
            stage_logs.append(
                {
                    ...
                    "required_corner_count": need_count,
                    "required_corner_v": need_corner_v,
                    "required_corner_d": need_corner_d,
                    "mode": expand_mode,
                    "a_max": a_work,
                    "b_max": b_work,
                    "a_norm": a_norm_work,
    ...
    if staged_k_relax:
        for kcur in range(ks, k_max + 1):
            if kcur > ks:
                a_work = max(a_work, a_norm_work << kcur)
                b_work = max(b_work, b_norm_work << kcur)
                ng, ncorner_ids = make_grid_graph(
                    corners,
                    a_max=a_work,
                    b_max=b_work,
    ...
    if final_prune and final_prune_rounds > 0 and final_prune_max_candidates > 0:
        ...
        for r in range(1, final_prune_rounds + 1):
            pruned, removed = _refresh_graph_by_pruning(
                best,
                corner_ids=corner_ids,
                max_deg=corner_max_deg,
                min_corner_lines=min_corner_lines,
            ...
            before_ck = priority_corner_kawasaki_score(best, corner_ids=corner_ids, tol=kawasaki_tol)
            ...
            keep_satisfied = _preserve_satisfied_corners(
                best,
                pruned,
                corner_ids=corner_ids,
                max_deg=corner_max_deg,
    ...
    if show_prune_axes and prune_axes_max > 0:
        reps = _collect_prune_axis_representatives(best, max_candidates=prune_axes_max)
    ...
    result: Dict[str, object] = {
        ...
        "params": {
            ...
            "expand_stall_rounds": expand_stall_rounds,
            "seed_auto_expand": seed_auto_expand,
            "seed_auto_expand_max_rounds": seed_auto_expand_max_rounds,
            "final_prune": final_prune,
            "final_prune_rounds": final_prune_rounds,
            "final_prune_max_candidates": final_prune_max_candidates,
            "show_prune_axes": show_prune_axes,
            "prune_axes_max": prune_axes_max,
            "show_order": show_order,
            "highlight_kawasaki": highlight_kawasaki,
...
def main() -> None:
    ...
    parser.add_argument("--a-max", type=int, default=2)
    ...
    parser.add_argument("--expand-stall-rounds", type=int, default=1)
    parser.add_argument("--no-seed-auto-expand", action="store_true")
    parser.add_argument("--seed-auto-expand-max-rounds", type=int, default=1)
    parser.add_argument("--no-final-prune", action="store_true")
    parser.add_argument("--final-prune-rounds", type=int, default=2)
    parser.add_argument("--final-prune-max-candidates", type=int, default=64)
    parser.add_argument("--show-prune-axes", action="store_true")
    parser.add_argument("--prune-axes-max", type=int, default=24)
    ...
    if not args.no_save_run_request:
        run_req = {
            ...
            "request": {
                ...
                "min_corner_lines": args.min_corner_lines,
                "kawasaki_tol": args.kawasaki_tol,
                "no_corner_kawasaki_repair": args.no_corner_kawasaki_repair,
                "triangle_macro": args.triangle_macro,
                "no_require_corner_kawasaki": args.no_require_corner_kawasaki,
                "staged_k_relax": args.staged_k_relax,
                "k_start": args.k_start,
                "dir_top_k": args.dir_top_k,
                "priority_top_n": args.priority_top_n,
                "local_ray_dirty": args.local_ray_dirty,
                "stop_on_corner_clear": args.stop_on_corner_clear,
                "show_order": args.show_order,
                "auto_expand_grid": args.auto_expand_grid,
                "auto_expand_max_rounds": args.auto_expand_max_rounds,
                "expand_stall_rounds": args.expand_stall_rounds,
                "no_seed_auto_expand": args.no_seed_auto_expand,
                "seed_auto_expand_max_rounds": args.seed_auto_expand_max_rounds,
                "no_final_prune": args.no_final_prune,
                "final_prune_rounds": args.final_prune_rounds,
                "final_prune_max_candidates": args.final_prune_max_candidates,
                "show_prune_axes": args.show_prune_axes,
                "prune_axes_max": args.prune_axes_max,
                "no_symmetry": args.no_symmetry,
                "no_open_sink": args.no_open_sink,
                "no_open_sink_repair": args.no_open_sink_repair,
                "highlight_kawasaki": args.highlight_kawasaki,
                "out_path": args.out_path,
                "no_render": args.no_render,
            },
        }
        out_dir = os.path.dirname(args.run_request_path)
        if out_dir:
            os.makedirs(out_dir, exist_ok=True)
        payload: List[Dict[str, object]] = []
        if os.path.exists(args.run_request_path):
            try:
                with open(args.run_request_path, "r", encoding="utf-8") as f:
                    existing = json.load(f)
                if isinstance(existing, list):
                    payload = [x for x in existing if isinstance(x, dict)]
                elif isinstance(existing, dict):
                    payload = [existing]
            except Exception:
                payload = []
        payload.append(run_req)
        with open(args.run_request_path, "w", encoding="utf-8") as f:
