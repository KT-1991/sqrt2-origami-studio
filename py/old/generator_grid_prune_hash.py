...
def _required_bounds_for_qsqrt2(z: Qsqrt2) -> Optional[Tuple[int, int, int]]:
    ...
    if aa.denominator != 1 or bb.denominator != 1:
        return None
    return (k, abs(aa.numerator), abs(bb.numerator))
...
class GridCreaseGraph:
    ...
    def recompute_ray_next_for_vertex(self, v: int) -> None:
        ...
        for d in range(ANGLE_COUNT):
            ...
            for i, j in self._iter_edges_for_ray_dir(d):
                hit_f = _ray_segment_hit_float(origin_f, d_f, self.points_f[i], self.points_f[j], eps=1e-12)
                if hit_f is None:
                    continue
                t_f, _ = hit_f
    ...
    def _mark_ray_dirty_by_changed_edges(self, changed_edges: Sequence[Tuple[int, int]]) -> None:
        ...
        for i, j in changed_edges:
            ii, jj = self._norm_edge(i, j)
            edge_segs.append((self.points_f[ii], self.points_f[jj]))
        for v in self.active_vertices:
            ...
            for d in range(ANGLE_COUNT):
                ...
                for a_f, b_f in edge_segs:
                    if _ray_segment_hit_float(origin_f, d_f, a_f, b_f, eps=1e-12) is not None:
                        self.ray_dirty.add(v)
                        hit_any = True
                        break
                if hit_any:
...
def _move_equiv_key(
    ...
    if enforce_symmetry:
        mv = find_vertex_idx(g, mirror_point_y_eq_x(g.points[v_idx]))
        if mv is None:
...
def seed_direct_corner_connections(
    ...
    for _, u, v in pairs:
        ...
        mirror_edge = e
        if enforce_symmetry:
            mu = find_vertex_idx(g, mirror_point_y_eq_x(g.points[u]))
            mv = find_vertex_idx(g, mirror_point_y_eq_x(g.points[v]))
...
def dfs_repair_corners(
    ...
    def recurse(state: GridCreaseGraph, depth: int) -> None:
        ...
        for v in priority[: max(1, priority_top_n)]:
            ...
            if enforce_symmetry:
                mirror_v = find_vertex_idx(state, mirror_point_y_eq_x(state.points[v]))
            for d in trial_dirs:
                ...
                if enforce_symmetry:
                    if mirror_v is None:
                        ...
                        md = mirrored_dir_idx(d)
                        mhit = state.ray_next_at(mirror_v, md)
...
def main() -> None:
    ...
    parser.add_argument("--local-ray-dirty", action="store_true")
