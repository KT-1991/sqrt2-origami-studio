...
class GridCreaseGraph:
    def __init__(
        ...
        points_proj_f: Optional[Sequence[Sequence[float]]] = None,
        ...
        if points_proj_f is None:
            self.points_proj_f = tuple(
                tuple(px * dx + py * dy for (px, py) in self.points_f)
                for dx, dy in DIRS_F
            )
        else:
            self.points_proj_f = points_proj_f
    ...
    def recompute_ray_next_for_vertex(self, v: int) -> None:
        ...
        points_proj_f = self.points_proj_f
    ...
    def _first_hit_edge_from_candidates(
        ...
        points_f = self.points_f
        proj = self.points_proj_f[dir_idx]
        ray_hit_t = _ray_segment_hit_t_float
        origin_f = points_f[origin_v]
        d_f = DIRS_F[dir_idx]
        origin_proj = proj[origin_v]
        d_i = DIRS_I[dir_idx]
