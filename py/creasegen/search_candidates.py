...
def collect_trial_dirs(
    ...
    *,
    ...
    preferred_dirs: Optional[List[int]],
    ...
    if preferred_dirs:
        ...
        for d in preferred_dirs:
            if d in seen_pref:
                continue
            seen_pref.add(d)
            if d in feasible_dirs:
                pref_order.append(d)
        if pref_order:
            stats_inc("hint_preferred_vertex", 1)
            cap = dir_top_k if dir_top_k > 0 else len(feasible_dirs)
            merged: List[int] = []
            for d in pref_order:
                merged.append(d)
                if len(merged) >= cap:
            ...
            if len(merged) < cap:
                for d in trial_dirs:
                    ...
                    if len(merged) >= cap:
                        break
            hint_used = sum(1 for d in merged if d in seen_pref)
            if hint_used > 0:
                stats_inc("hint_preferred_dir_used", hint_used)
            trial_dirs = merged
