...
def required_corner_lines(
    ...
    *,
    ...
    if max_deg <= 0.0:
        raise ValueError("max_deg must be positive")
    px, py = g.points_f[v_idx]
    _, width = _interior_wedge(px, py)
    thr = max_deg * pi / 180.0
    need_sectors = max(1, ceil((width - 1e-12) / thr))
    if width >= 2 * pi - 1e-10:
        need_lines = need_sectors
    else:
        need_lines = max(0, need_sectors - 1)
    return max(min_corner_lines, need_lines)
...
def corner_score(
    ...
    lowdeg_pen = 0.0
    for v in corner_ids:
        need_lines = required_corner_lines(
            g,
            v,
            max_deg=max_deg,
            min_corner_lines=min_corner_lines,
        )
        deficit = max(0, need_lines - corner_line_count(g, v))
        if deficit > 0:
            lowdeg += 1
